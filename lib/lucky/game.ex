defmodule Game do
  ...
  def init(_) do
    {:ok,
     %{dice: nil, bet: nil, state: :unstarted, players: [], did_peek: false, blind_bet: false}}
  end
  ...
  def handle_call({:add_player, player}, _from, game) do
    exists_player? =
      not Enum.empty?(Enum.filter(game.players, fn pl -> pl.name == player.name end))
    ...
    {ok, game} =
      case exists_player? do
        ...
        false ->
          max_ord =
            case Enum.max_by(game.players, fn pl -> pl.ord end, &>=/2, fn -> 0 end) do
              %{ord: max_ord} -> max_ord
              _ -> 0
            end

          player = Map.put(player, :ord, max_ord + 1)
          player = Map.put(player, :loss_count, 0)
      ...
      case exists_player? do
        ...
        true ->
          ...
          case gm_player do
            %{is_current: false} ->
              {:ok,
               %{game | players: Enum.filter(game.players, fn pl -> pl.name != player.name end)}}
            ...
            %{is_current: true} ->
              {:ok,
               %{
                 game
                 | players:
                     Enum.filter(set_next(game.players), fn pl -> pl.name != player.name end),
                   bet: nil,
                   dice: nil,
                   state: :pre_roll,
                   did_peek: false,
  ...
  def handle_call({:bet, bet}, _from, game) do
    ...
    game =
      case bet_is_higher(bet, game.bet) do
        false ->
          game
        ...
        true ->
          case game.state do
            :roll_1 ->
              %{game | bet: bet, state: :passed, players: set_next(game.players), blind_bet: true}
  ...
  def handle_call(:reject, _from, game) do
    {bet, dice, loser, game} =
      case bet_is_higher(game.dice, game.bet) or bet_is_equal(game.bet, game.dice) do
        true ->
          loser = get_current(game.players)

          {game.bet, game.dice, loser,
           %{
             game
             | players: record_loss(loser, game.players),
               state: :curr_loss,
               bet: nil,
               dice: nil,
               did_peek: false,
        ...
        false ->
          loser = get_previous(game.players)
  ...
  defp record_loss(loser, players) do
    players
    |> Enum.map(fn player ->
      case player do
        ^loser -> %{loser | loss_count: loser.loss_count + 1}
        _ -> player
      end
    end)
  end
